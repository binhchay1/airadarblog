<?php
/**
 * Define the auto update functionality
 *
...
if ( ! class_exists( 'Easy_Accordion_Pro_Auto_Update' ) ) {
	 ...
	 * @version 1.6.19
	 */
	class Easy_Accordion_Pro_Auto_Update {
		/**
		 * Api_url
		 *
		 * @var string
		 ...
		 * @param array  $plugin The plugins.
		 */
		public function show_update_notification( $file, $plugin ) {
			...
			if ( ! empty( $update_cache->response[ $this->name ] ) && version_compare( $this->version, $version_info->new_version, '<' ) ) {
				...
				if ( empty( $version_info->download_link ) ) {
					printf(
						__( 'There is a new version of %1$s available. %2$sView version %3$s details%4$s.', 'easy-accordion-pro' ),
					...
					printf(
						__( 'There is a new version of %1$s available. %2$sView version %3$s details%4$s or %5$supdate now%6$s.', 'easy-accordion-pro' ),
						esc_html( $version_info->name ),
						'<a target="_blank" class="thickbox" href="' . esc_url( $changelog_link ) . '">',
						esc_html( $version_info->new_version ),
						'</a>',
		...
		public function show_changelog() {
			...
			if ( ! current_user_can( 'update_plugins' ) ) {
				wp_die( __( 'You do not have permission to install plugin updates', 'easy-accordion-pro' ), __( 'Error', 'easy-accordion-pro' ), array( 'response' => 403 ) );
			}
