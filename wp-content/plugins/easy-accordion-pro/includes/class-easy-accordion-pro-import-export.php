 ...
 * Custom import export.
 */
class Easy_Accordion_Pro_Import_Export {
	...
	public function export( $accordion_ids ) {
		...
		if ( ! empty( $accordion_ids ) ) {
			$post_in    = 'all_accordion' === $accordion_ids ? '' : $accordion_ids;
			$args       = array(
	...
	public function export_accordions() {
		...
		$accordion_ids = '';
		if ( isset( $_POST['eap_ids'] ) ) {
			$accordion_ids = is_array( $_POST['eap_ids'] ) ? wp_unslash( array_map( 'absint', $_POST['eap_ids'] ) ) : sanitize_text_field( wp_unslash( $_POST['eap_ids'] ) );
		}
	...
	public function import_accordions() {
		...
		if ( ! wp_verify_nonce( $nonce, 'eapro_options_nonce' ) ) {
			wp_send_json_error(
				array(
					'error' => esc_html__( 'Error: Nonce verification has failed. Please try again.', 'easy-accordion-pro' ),
				),
				403
			);
		}
		...
		$accordions = $allow_tags ? $data['accordion'] : wp_kses_post_deep( $data['accordion'] );
		if ( ! $data ) {
			wp_send_json_error(
				array(
					'message' => __( 'Nothing to import.', 'easy-accordion-pro' ),
				),
				400
			);
		}
