<?php
/**
 * The option class functionality of the plugin.
 *
...
if ( ! class_exists( 'SP_EAP_Options' ) ) {
	...
	class SP_EAP_Options extends SP_EAP_Abstract {
		...
		public function ajax_save() {
			...
			if ( ! $result ) {
				wp_send_json_error( array( 'error' => esc_html__( 'Error while saving.', 'easy-accordion-pro' ) ) );
			} else {
		...
		public function set_options( $ajax = false ) {
			...
			if ( wp_verify_nonce( $nonce, 'eapro_options_nonce' ) ) {
				...
				if ( ! $ajax && ! empty( $response['eapro_import_data'] ) ) {
					...
					$importing    = true;
					$this->notice = esc_html__( 'Success. Imported backup options.', 'easy-accordion-pro' );
				...
				if ( ! empty( $transient['reset'] ) ) {
					...
					$this->notice = esc_html__( 'Default options restored.', 'easy-accordion-pro' );
					...
					$this->notice = esc_html__( 'Default options restored for only this section.', 'easy-accordion-pro' );
				...
				if ( empty( $this->notice ) ) {
					$this->notice = esc_html__( 'Settings saved.', 'easy-accordion-pro' );
				}
		...
		public function add_admin_menu() {
			...
			$menu_capability = apply_filters( 'sp_easy_accordion_ui_permission', 'manage_options' );
		...
		public function add_options_html() {
			...
			if ( $show_buttons ) {
				echo '<h1><img src="' . esc_url( SP_EAP_URL ) . '/admin/img/ea-icon.svg" alt="">' . esc_html( $this->args['framework_title'] ) . '</h1>';
			} else {
				echo '<h1><img src="' . esc_url( SP_EAP_URL ) . '/admin/img/import-export.svg" alt="">' . esc_html( $this->args['framework_title'] ) . '</h1>';
			}
			...
			echo ( $has_nav && $this->args['show_all_options'] ) ? '<div class="eapro-expand-all" title="' . esc_html__( 'show all options', 'easy-accordion-pro' ) . '"><i class="fa fa-outdent"></i></div>' : '';

			echo ( $this->args['show_search'] ) ? '<div class="eapro-search"><input type="text" name="eapro-search" placeholder="' . esc_html__( 'Search option(s)', 'easy-accordion-pro' ) . '" autocomplete="off" /></div>' : '';
			if ( $show_buttons ) {
				echo '<div class="eapro-buttons">';
				echo '<input type="submit" name="' . esc_attr( $this->unique ) . '[_nonce][save]" class="button button-primary eapro-top-save eapro-save' . esc_attr( $ajax_class ) . '" value="' . esc_html__( 'Save Settings', 'easy-accordion-pro' ) . '" data-save="' . esc_html__( 'Saving...', 'easy-accordion-pro' ) . '">';
				echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="eapro_transient[reset_section]" class="button button-secondary eapro-reset-section eapro-confirm" value="' . esc_html__( 'Reset Tab', 'easy-accordion-pro' ) . '" data-confirm="' . esc_html__( 'Are you sure to reset all the settings of this tab?', 'easy-accordion-pro' ) . '">' : '';
				...
				if ( ! empty( $section['fields'] ) ) {
					...
					foreach ( $section['fields'] as $field ) {
						...
						SP_EAP::field( $field, $value, $this->unique, 'options' );
								...
								echo '<div class="eapro-no-option eapro-text-muted">' . esc_html__( 'No option provided by developer.', 'easy-accordion-pro' ) . '</div>';
