<?php
/**
 * The setup class functionality of the plugin.
 *
...
if ( ! class_exists( 'SP_EAP' ) ) {
	...
	class SP_EAP {
		 ...
		 *
		 * Uses the Easy_Accordion_Pro_I18n class in order to set the domain and to register the hook
		 * with WordPress.
		 *
		 * @since    2.0.0
		 * @access   private
		...
		public static function set_locale() {
			require_once SP_EAP_INCLUDES . '/class-easy-accordion-pro-i18n.php';
			$plugin_i18n = new Easy_Accordion_Pro_I18n();
		 ...
		 *
		 * @param  mixed $id id.
		 * @param  mixed $sections arguments.
		 * @return void
		 */
		...
		public static function include_plugin_url( $file ) {
			return esc_url( SP_EAP_URL . '/admin/views/models' ) . '/' . ltrim( $file, '/' );
		}
		...
		public static function add_admin_enqueue_scripts( $hook ) {
			...
			if ( 'edit.php' === $hook || 'post-new.php' === $hook || 'post.php' === $hook ) {
				if ( 'product' === $the_current_post_type ) {
					wp_enqueue_script( 'sp-woo-js', SP_EAP_URL . 'admin/js/woo-tab.min.js', array( 'jquery' ), SP_EAP_VERSION, true );
				}
			}
			if ( 'sp_easy_accordion' === $the_current_post_type ) {
				...
				if ( apply_filters( 'eapro_fa4', true ) ) {
					wp_enqueue_style( 'sp-ea-font-awesome' );
				} else {
					wp_enqueue_style( 'eapro-fa5', 'https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@5.13.0/css/all' . $min . '.css', array(), SP_EAP_VERSION, 'all' );
					wp_enqueue_style( 'eapro-fa5-v4-shims', 'https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@5.13.0/css/v4-shims' . $min . '.css', array(), SP_EAP_VERSION, 'all' );
				}

				// framework core styles.
				wp_enqueue_style( 'eapro', self::include_plugin_url( 'assets/css/eapro' . $min . '.css' ), array(), SP_EAP_VERSION, 'all' );

				// rtl styles.
				if ( is_rtl() ) {
					wp_enqueue_style( 'eapro-rtl', self::include_plugin_url( 'assets/css/eapro-rtl' . $min . '.css' ), array(), SP_EAP_VERSION, 'all' );
				}

				// framework core scripts.
				wp_enqueue_script( 'eapro-plugins', self::include_plugin_url( 'assets/js/eapro-plugins' . $min . '.js' ), array(), SP_EAP_VERSION, true );
				wp_enqueue_script( 'eapro', self::include_plugin_url( 'assets/js/eapro' . $min . '.js' ), array( 'eapro-plugins' ), SP_EAP_VERSION, true );

				wp_localize_script(
					'eapro',
					'eapro_vars',
					array(
						'pluginsUrl'    => SP_EAP_URL,
						'color_palette' => apply_filters( 'eapro_color_palette', array() ),
						'i18n'          => array(
							// global localize.
							'confirm'             => esc_html__( 'Are you sure?', 'easy-accordion-pro' ),
							'reset_notification'  => esc_html__( 'Restoring options.', 'easy-accordion-pro' ),
							'import_notification' => esc_html__( 'Importing options.', 'easy-accordion-pro' ),

							// chosen localize.
							'typing_text'         => esc_html__( 'Please enter or more characters', 'easy-accordion-pro' ),
							'searching_text'      => esc_html__( 'Searching...', 'easy-accordion-pro' ),
							'no_results_text'     => esc_html__( 'No results match', 'easy-accordion-pro' ),
						),
					)
				);
		...
		public static function field( $field = array(), $value = '', $unique = '', $where = '', $parent = '' ) {
			...
			if ( ! empty( $field['_notice'] ) ) {
				...
				$field            = array();
				$field['content'] = sprintf( esc_html__( 'Ooops! This field type (%s) can not be used here, yet.', 'easy-accordion-pro' ), '<strong>' . $field_type . '</strong>' );
				$field['type']    = 'notice';
				$field['style']   = 'danger';
			...
			if ( ! empty( $field_type ) ) {
				...
				echo '<div class="eapro-field eapro-field-' . esc_attr( $field_type . $is_pseudo . $class . $hidden ) . '"' . wp_kses_post( $depend ) . '>';
				...
				if ( class_exists( $classname ) ) {
					...
					echo '<p>' . esc_html__( 'This field class is not available!', 'easy-accordion-pro' ) . '</p>';
				}
			} else {
				echo '<p>' . esc_html__( 'This type is not found!', 'easy-accordion-pro' ) . '</p>';
			}
