	...
	class SP_EAP_Field_checkbox extends SP_EAP_Fields {
		...
		public function render() {
			...
			if ( isset( $this->field['options'] ) ) {
					...
					foreach ( $options as $option_key => $option_value ) {
						...
						if ( is_array( $option_value ) && ! empty( $option_value ) ) {
							...
							foreach ( $option_value as $sub_key => $sub_value ) {
								...
								echo '<li><label><input type="checkbox" name="' . esc_attr( $this->field_name( '[]' ) ) . '" value="' . esc_attr( $sub_key ) . '"' . $this->field_attributes() . esc_attr( $checked ) . '/> ' . esc_attr( $sub_value ) . '</label></li>'; // phpcs:ignore 
							}
							echo '</ul>';
							echo '</li>';
							...
							echo '<li><label><input type="checkbox" name="' . esc_attr( $this->field_name( '[]' ) ) . '" value="' . esc_attr( $option_key ) . '"' . $this->field_attributes() . $checked . '/> ' . esc_attr( $option_value ) . '</label></li>'; // phpcs:ignore 
				...
				echo '<input type="hidden" name="' . esc_attr( $this->field_name() ) . '" value="' . $this->value . '" class="eapro--input"' . $this->field_attributes() . '/>'; // phpcs:ignore 
