	...
	class SP_EAP_Field_select extends SP_EAP_Fields {
		...
		public function render() {
			...
			if ( isset( $this->field['options'] ) ) {
				...
				if ( ! empty( $args['ajax'] ) ) {
					global $post;
					$upload_data                       = get_post_meta( $post->ID, 'sp_eap_upload_options', true );
					$eap_post_type                     = isset( $upload_data['eap_post_type'] ) ? $upload_data['eap_post_type'] : $args['options'];
					$args['settings']['data']['type']  = $eap_post_type;
				...
				if ( ( is_array( $options ) && ! empty( $options ) ) || ( ! empty( $args['chosen'] ) && ! empty( $args['ajax'] ) ) ) {
					...
					if ( ! empty( $args['chosen'] ) && ! empty( $args['multiple'] ) ) {
						...
						echo '<select name="' . $field_name . '" class="eapro-hidden-select eapro-hidden"' . $multiple_attr . $field_attr . '>';//phpcs:ignore
