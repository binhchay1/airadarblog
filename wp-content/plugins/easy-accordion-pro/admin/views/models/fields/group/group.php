		...
		public function render() {
			...
			$args = wp_parse_args(
				...
				array(
					...
					'min'                    => 0,
					'fields'                 => array(),
					'button_title'           => esc_html__( 'Add New', 'easy-accordion-pro' ),
			...
			if ( ! empty( $this->parent ) && preg_match( '/' . preg_quote( '[' . $this->field['id'] . ']' ) . '/', $this->parent ) ) {
				...
				echo '<div class="eapro-notice eapro-notice-danger">' . esc_html__( 'Error: Nested field id can not be same with another nested field id.', 'easy-accordion-pro' ) . '</div>';
				...
				echo '<i class="eapro-cloneable-sort fa fa-arrows-alt"></i>';
				echo '<i class="eapro-cloneable-clone fa fa-clone"></i>';
				echo '<i class="eapro-cloneable-remove eapro-confirm fa fa-times" data-confirm="' . esc_html__( 'Are you sure to delete this item?', 'easy-accordion-pro' ) . '"></i>';
				echo '</div>';
				...
				if ( ! empty( $this->value ) ) {
					...
					foreach ( $this->value as $value ) {
						...
						echo '<i class="eapro-cloneable-remove eapro-confirm fa fa-times" data-confirm="' . esc_html__( 'Are you sure to delete this item?', 'easy-accordion-pro' ) . '" title="Remove"></i>';
						echo '</div>';
				...
				echo '<div class="eapro-cloneable-alert eapro-cloneable-max">' . esc_html__( 'You can not add more than', 'easy-accordion-pro' ) . ' ' . esc_attr( $args['max'] ) . '</div>';
				echo '<div class="eapro-cloneable-alert eapro-cloneable-min">' . esc_html__( 'You can not remove less than', 'easy-accordion-pro' ) . ' ' . esc_attr( $args['min'] ) . '</div>';
