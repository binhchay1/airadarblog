; (function ($, window, document, undefined) {
	...
	// Field: icon
	//
	$.fn.eapro_field_icon = function () {
		return this.each(function () {

			var $this = $(this);

			$this.on('click', '.eapro-icon-add', function (e) {

				e.preventDefault();

				var $button = $(this);
				var $modal = $('#eapro-modal-icon');

				$modal.show();

				SP_EAP.vars.$icon_target = $this;

				if (!SP_EAP.vars.icon_modal_loaded) {

					$modal.find('.eapro-modal-loading').show();

					window.wp.ajax.post('eapro-get-icons', {
						nonce: $button.data('nonce')
					}).done(function (response) {

						$modal.find('.eapro-modal-loading').hide();

						SP_EAP.vars.icon_modal_loaded = true;

						var $load = $modal.find('.eapro-modal-load').html(response.content);
						...
						$load.on('click', 'i', function (e) {
							...
							SP_EAP.vars.$icon_target.find('i').removeAttr('class').addClass(icon);
							SP_EAP.vars.$icon_target.find('input').val(icon).trigger('change');
							SP_EAP.vars.$icon_target.find('.eapro-icon-preview').removeClass('hidden');
							SP_EAP.vars.$icon_target.find('.eapro-icon-remove').removeClass('hidden');

							$modal.hide();

						});

						$modal.on('change keyup', '.eapro-icon-search', function () {
						...
						$modal.find('.eapro-modal-loading').hide();
						$modal.find('.eapro-modal-load').html(response.error);
			...
			$this.on('click', '.eapro-icon-remove', function (e) {
				e.preventDefault();
				$this.find('.eapro-icon-preview').addClass('hidden');
	...
	$.fn.eapro_field_media = function () {
		return this.each(function () {
			...
			var $this = $(this),
				...
				$auto_attributes = ($this.hasClass('eapro-assign-field-background')) ? $this.closest('.eapro-field-background').find('.eapro--auto-attributes') : false,
				wp_media_frame;
			...
			$upload_button.on('click', function (e) {
				...
				wp_media_frame.on('select', function () {
					...
					$this.find('.eapro--id').val(attributes.id);
					$this.find('.eapro--width').val(attributes.width);
					$this.find('.eapro--height').val(attributes.height);
					$this.find('.eapro--alt').val(attributes.alt);
					$this.find('.eapro--title').val(attributes.title);
					$this.find('.eapro--description').val(attributes.description);
					...
					if ($auto_attributes) {
						$auto_attributes.removeClass('eapro--attributes-hidden');
					}

					$remove_button.removeClass('hidden');

					$this.find('.eapro--preview').removeClass('hidden');
					$this.find('.eapro--src').attr('src', thumbnail);
					$this.find('.eapro--thumbnail').val(thumbnail);
					$this.find('.eapro--url').val(attributes.url).trigger('change');
			...
			$remove_button.on('click', function (e) {
				...
				if ($auto_attributes) {
					$auto_attributes.addClass('eapro--attributes-hidden');
				}

				$remove_button.addClass('hidden');
				$this.find('input').val('');
				$this.find('.eapro--preview').addClass('hidden');
				$this.find('.eapro--url').trigger('change');
	...
	$.fn.eapro_reload_script = function (options) {
		...
		return this.each(function () {
			...
			if (!$this.data('inited')) {
				...
				$this.children('.eapro-field-icon').eapro_field_icon();
				$this.children('.eapro-field-media').eapro_field_media();
				...
				$('.eap-admin-header').find('.ea-support-area')
					.eapro_help()
	...
	$('.sp_eap_post_type select').on('change', function (event) {
		event.preventDefault();
		var data = {
			action: 'sp_eap_get_taxonomies',
			eap_post_type: $(this).val(),
		}
		$.post(ajaxurl, data, function (resp) {
			$('.sp_eap_post_taxonomy select').html(resp);
			$(".sp_eap_post_taxonomy select").trigger("chosen:updated");
		});
	});

	// Update terms list on the change of post taxonomy.
	$('.sp_eap_post_taxonomy select').on('change', function (event) {
		event.preventDefault();
		var data = {
			action: 'sp_eap_get_terms',
			eap_post_taxonomy: $(this).val(),
		}

		$.post(ajaxurl, data, function (resp) {
			$('.sp_eap_taxonomy_terms select').html(resp);
			$(".sp_eap_taxonomy_terms select").trigger("chosen:updated");
		});
	});
	...
	$('.eap_export .eapro--button').on('click', function (event) {
		...
		var $accordion_ids_type = 'all_accordion' === $export_type ? 'all_accordion' : $accordion_ids;
		var $ex_nonce = $('#eapro_options_noncesp_eap_tools').val();
		if ('all_accordion' === $export_type || 'selected_accordion' === $export_type) {
			var data = {
	...
	$('.eap_import button.import').on('click', function (event) {
		...
		var $this = $(this),
			...
			eap_accordions = $('#import').prop('files')[0];
			...
			reader.onload = function (event) {
				...
				$.ajax({
					...
					error: function (error) {
						$('.eapro-form-result.eapro-form-success').text('Something went wrong!').addClass('eapro-import-warning').show();
						$this.prop('disabled', false).css('opacity', '1').text(button_text);
						setTimeout(function () {
							$('.eapro-form-result.eapro-form-success').hide().removeClass('eapro-import-warning').text('');
							$('#import').val('');
						}, 2000);
					}
				});
	...
	$('.sp_eap_post_type select').on('change', function (event) {
		event.preventDefault();
		var data = {
			action: 'sp_eap_get_posts',
			eap_post_type: $(this).val(),
		}
		$.post(ajaxurl, data, function (resp) {
			$('.sp_eap_specific_posts select').html(resp);
			$(".sp_eap_specific_posts select").trigger("chosen:updated");
		});
	});
	...
	$('.eap-shortcode-selectable').on('click', function (e) {
		...
		$(this).focus().select();
		jQuery(".sp_eap-after-copy-text:not(.sp-eap-pagination-not-work)").animate({
			opacity: 1,
			bottom: 25
		}, 300);
		setTimeout(function () {
	...
	// Theme preview
	function updateThemePreviewImage(selector, regex) {
		var str = "";
	...
	$('.sp_eap_accordion_theme').on('change', function () {
		updateThemePreviewImage(".sp_eap_accordion_theme", /theme-preview\/(.+)\.svg/);
	});
	...
	$(document).on('click', '#sp__eap-show-preview:not(:contains(Hide))', function (e) {
		...
		$.ajax({
			...
			success: function (response) {
				...
				$('.sp-eap-load-more button, .sp-eap-pagination-number a.sp-eap-page-numbers').on('click', function (e) {
					e.preventDefault();
					e.stopPropagation();
					e.stopImmediatePropagation();
					$('.sp-eap-pagination-not-work').animate({
						opacity: 1,
						bottom: 25
					}, 300);
					setTimeout(function () {
						jQuery(".sp-eap-pagination-not-work").animate({
							opacity: 0,
						}, 200);
						jQuery(".sp-eap-pagination-not-work").animate({
							bottom: 0
