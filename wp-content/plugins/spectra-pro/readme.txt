...
= 1.1.3 - WEDNESDAY, 28th FEBRUARY 2024 =
* Improvement: Countdown - Added toggles to automatically reload the page or content when the countdown expires.
* Improvement: Image Gallery - Added a toggle to open custom URLs in a new tab.
* Improvement: Modal - Improved the accessibility of the off-canvas modals.
* Fix: Modal - Resolved an issue wherein the close button background color would not work as intended for off-canvas modals.
* Fix: Modal - Resolved an issue wherein the modal would only be triggered by the first element when multiple class-trigger elements are created.
* Fix: Dynamic Content - Resolved an issue wherein post excerpts would not load as intended when Dynamic Content was enabled.
* Fix: Dynamic Content - Added dynamic content support to image block title. 

= 1.1.2 - MONDAY, 8th JANUARY 2024 =
* Fix: Container - Resolved an issue wherein the overlay opacity would be doubled when using a dynamic background image.
* Fix: Image Gallery - Resolved an issue wherein custom URLs without 'www' could appear as invalid in some cases.
* Fix: Dynamic Content - Resolved an issue wherein the disable link would not work as intended for the include-context.
